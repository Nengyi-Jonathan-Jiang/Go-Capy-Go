const levels = [
/*
@name "Where Am I?"
@background "tutor-A-background"

*/
new Level("Where Am I?", document.getElementById("tutor-A-background")),
/*
@name "Doors and Buttons"
@background "tutor-B-background"

platform 2 5 2 0.5
box 2.5 4
platform 6 -1 1 6.5

control #fac
button #fac 0 6
door #fac 6 5.5 1 1.5

control #4fc
button #4fc 8 6
door #4fc 9 2 1 -5
platform 10 2 1 6
*/
new Level("Doors and Buttons", document.getElementById("tutor-B-background"), function(level, player, berry){
    level.addPlatformAt(4, 6.25, 2, 0.5);
    level.addPlatformAt(7.5, 3.25, 1, 6.5);
    level.addPlatformAt(11.5, 6, 1, 6);
    level.addBoxAt(4, 5.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(7.5, 7.25, 1, 1.5))
    doorManager0.linkButton(level.addButtonAt(1.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(10.5, 5.5, 1, -5))
    doorManager1.linkButton(level.addButtonAt(9.5, 8))
}),
/*
@name "Ghostly Helper"
@background "tutor-C-background"

platform 4 5 11 0.5
pumpkin 13 4

platform 4 5 1 1
platform 7 5 1 1
platform 10 5 1 1

control #fac
button #fac 10 4
door #fac 4 6 1 1

control #4fc
button #4fc 7 4
door #4fc 7 6 1 1

control #fd4
button #fd4 4 4
door #fd4 10 6 1 1
*/
new Level("Ghostly Helper", document.getElementById("tutor-C-background"), function(level, player, berry){
    level.addPlatformAt(10.5, 6.25, 11, 0.5);
    level.addPlatformAt(5.5, 6.5, 1, 1);
    level.addPlatformAt(8.5, 6.5, 1, 1);
    level.addPlatformAt(11.5, 6.5, 1, 1);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(5.5, 7.5, 1, 1))
    doorManager0.linkButton(level.addButtonAt(11.5, 6))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(8.5, 7.5, 1, 1))
    doorManager1.linkButton(level.addButtonAt(8.5, 6))
    let doorManager2 = level.addDoorManager("#fd4");
    doorManager2.linkDoor(level.addDoorAt(11.5, 7.5, 1, 1))
    doorManager2.linkButton(level.addButtonAt(5.5, 6))
}, function(level){
    level.addPumpkinAt(14.5, 5.4);
}),
/*
@name "Semipermeable"
@background "level-4-background"

control #fac
control #fd4
control #caf

platform 2 5 1 2

platform 3 4.5 2 0.5
platform 4 3 6 0.5
box 4 3.5
block 5 4.5
block 6 4.5
block 7 4.5
platform 8 4.5 1 2.5
platform 10 4.5 1 2.5
platform 10 0 1 3.5
platform 4 5 1 2

door #fac 10 3.5 1 1
button #fac 9 6

button #fd4 2 4
door #fd4 3 6 1 -1

button #caf  3 5
box 3 5
door #caf 5 4.5 3 -1

pumpkin 9 2
*/
new Level("Semipermeable", document.getElementById("tutor-D-background"), function(level, player, berry){
    level.addPlatformAt(3.5, 7, 1, 2);
    level.addPlatformAt(5, 5.75, 2, 0.5);
    level.addPlatformAt(8, 4.25, 6, 0.5);
    level.addPlatformAt(9.5, 6.75, 1, 2.5);
    level.addPlatformAt(11.5, 6.75, 1, 2.5);
    level.addPlatformAt(11.5, 2.75, 1, 3.5);
    level.addPlatformAt(5.5, 7, 1, 2);
    level.addBoxAt(5.5, 5);
    level.addBoxAt(4.5, 6.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(11.5, 5, 1, 1))
    doorManager0.linkButton(level.addButtonAt(10.5, 8))
    let doorManager1 = level.addDoorManager("#fd4");
    doorManager1.linkDoor(level.addDoorAt(4.5, 7.5, 1, -1))
    doorManager1.linkButton(level.addButtonAt(3.5, 6))
    let doorManager2 = level.addDoorManager("#caf");
    doorManager2.linkDoor(level.addDoorAt(7.5, 6, 3, -1))
    doorManager2.linkButton(level.addButtonAt(4.5, 7))
    level.addBlockAt(6.5, 6);
    level.addBlockAt(7.5, 6);
    level.addBlockAt(8.5, 6);
}, function(level){
    level.addPumpkinAt(10.5, 3.4);
}),
/*
@name "Cake Walk"
@background "level-1-background"

control #fd4
control #4fc
control #fac

button #fd4 0 6
button #4fc 1 6
button #fac 2 6

pumpkin 1 3
box 3 4
platform 3 3.5 12 0.5
platform 3 5 1 2

block 4 5
door #fac 4 4 1 1
door #fd4 4 5 2 -1
block 5 5

block 6 5
door #4fc 6 4 1 1
door #fac 6 5 2 -1
block 7 5

block 8 5
door #fd4 8 4 1 1
door #4fc 8 5 2 -1
block 9 5

platform 10 5 1 2

control #caf
door #caf 11 5 1 -2
platform 12 5 2 1
block 12 4
button #caf 13 4
*/
new Level("Cake Walk", document.getElementById("level-1-background"), function(level, player, berry){
    level.addPlatformAt(10, 4.75, 12, 0.5);
    level.addPlatformAt(4.5, 7, 1, 2);
    level.addPlatformAt(11.5, 7, 1, 2);
    level.addPlatformAt(14, 6.5, 2, 1);
    level.addBoxAt(4.5, 5.5);
    let doorManager0 = level.addDoorManager("#fd4");
    doorManager0.linkDoor(level.addDoorAt(6, 6.5, 2, -1))
    doorManager0.linkDoor(level.addDoorAt(9.5, 5.5, 1, 1))
    doorManager0.linkButton(level.addButtonAt(1.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(7.5, 5.5, 1, 1))
    doorManager1.linkDoor(level.addDoorAt(10, 6.5, 2, -1))
    doorManager1.linkButton(level.addButtonAt(2.5, 8))
    let doorManager2 = level.addDoorManager("#fac");
    doorManager2.linkDoor(level.addDoorAt(5.5, 5.5, 1, 1))
    doorManager2.linkDoor(level.addDoorAt(8, 6.5, 2, -1))
    doorManager2.linkButton(level.addButtonAt(3.5, 8))
    let doorManager3 = level.addDoorManager("#caf");
    doorManager3.linkDoor(level.addDoorAt(12.5, 7, 1, -2))
    doorManager3.linkButton(level.addButtonAt(14.5, 6))
    level.addBlockAt(5.5, 6.5);
    level.addBlockAt(6.5, 6.5);
    level.addBlockAt(7.5, 6.5);
    level.addBlockAt(8.5, 6.5);
    level.addBlockAt(9.5, 6.5);
    level.addBlockAt(10.5, 6.5);
    level.addBlockAt(13.5, 5.5);
}, function(level){
    level.addPumpkinAt(2.5, 4.4);
}),
/*
@name "Double Entendre"
@background "level-2-background"

control #fac

button #fac 0 6
button #fac 3 6
button #fac 6 6

platform 7 5 1 2
platform 7 0 1 2
platform 2.5 5 2 0.5
box 3 4
door #fac 7 2 1 3
pumpkin 3 2.5
*/
new Level("Double Entendre", document.getElementById("level-2-background"), function(level, player, berry){
    level.addPlatformAt(8.5, 7, 1, 2);
    level.addPlatformAt(8.5, 2, 1, 2);
    level.addPlatformAt(4.5, 6.25, 2, 0.5);
    level.addBoxAt(4.5, 5.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(8.5, 4.5, 1, 3))
    doorManager0.linkButton(level.addButtonAt(1.5, 8))
    doorManager0.linkButton(level.addButtonAt(4.5, 8))
    doorManager0.linkButton(level.addButtonAt(7.5, 8))
}, function(level){
    level.addPumpkinAt(4.5, 3.9);
}),
/*
@name "Too Slippery"
@background "level-3-background"

control #fac
button #fac 0 6
button #fac 4 6
door #fac 7 6 1 1
platform 2 5 3 0.5
platform 7 5 1 1
platform 7 3 4 0.5
control #4fc
button #4fc 9 6
door #4fc 7 3.5 1 1.5
door #4fc 7 1 1 -2
platform 10 3 1 5
platform 10 2 1 1
control #fd4
door #fd4 10 0 1 2
button #fd4 9 2
box 3 4
box 3 6
box 7 0
*/
new Level("Too Slippery", document.getElementById("level-3-background"), function(level, player, berry){
    level.addPlatformAt(4.5, 6.25, 3, 0.5);
    level.addPlatformAt(8.5, 6.5, 1, 1);
    level.addPlatformAt(10, 4.25, 4, 0.5);
    level.addPlatformAt(11.5, 6.5, 1, 5);
    level.addPlatformAt(11.5, 3.5, 1, 1);
    level.addBoxAt(4.5, 5.5);
    level.addBoxAt(4.5, 7.5);
    level.addBoxAt(8.5, 1.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(8.5, 7.5, 1, 1))
    doorManager0.linkButton(level.addButtonAt(1.5, 8))
    doorManager0.linkButton(level.addButtonAt(5.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(8.5, 5.25, 1, 1.5))
    doorManager1.linkDoor(level.addDoorAt(8.5, 3, 1, -2))
    doorManager1.linkButton(level.addButtonAt(10.5, 8))
    let doorManager2 = level.addDoorManager("#fd4");
    doorManager2.linkDoor(level.addDoorAt(11.5, 2, 1, 2))
    doorManager2.linkButton(level.addButtonAt(10.5, 4))
}),
/*
@name "UNKNOWN"
@background "level-4-background"
*/
new Level("UNKNOWN", document.getElementById("level-4-background")),
/*
@name "Backtracking"
@background "level-5-background"

control #fac
control #4fc
control #fd4

button #fac 4 6
door #fac 5 1 1 -6

button #4fc 4 0
door #4fc 6 1 1 -6
box 6 0

platform 4 1 1 0.5
control #caf
door #caf 7 1 1 -6
button #caf 8 6

door #fd4 10 6 1 1
button #fd4 9 6
button #fd4 0 6
platform 10 0 1 6
platform 12 1 1 5
block 12 6

door #4fc 11 1 1 -6

pumpkin 2 4
platform 1.5 5 2 .5
*/
new Level("Backtracking", document.getElementById("level-5-background"), function(level, player, berry){
    level.addPlatformAt(5.5, 2.25, 1, 0.5);
    level.addPlatformAt(11.5, 4, 1, 6);
    level.addPlatformAt(13.5, 4.5, 1, 5);
    level.addPlatformAt(3.5, 6.25, 2, 0.5);
    level.addBoxAt(7.5, 1.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(6.5, 5, 1, -6))
    doorManager0.linkButton(level.addButtonAt(5.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(7.5, 5, 1, -6))
    doorManager1.linkDoor(level.addDoorAt(12.5, 5, 1, -6))
    doorManager1.linkButton(level.addButtonAt(5.5, 2))
    let doorManager2 = level.addDoorManager("#fd4");
    doorManager2.linkDoor(level.addDoorAt(11.5, 7.5, 1, 1))
    doorManager2.linkButton(level.addButtonAt(10.5, 8))
    doorManager2.linkButton(level.addButtonAt(1.5, 8))
    let doorManager3 = level.addDoorManager("#caf");
    doorManager3.linkDoor(level.addDoorAt(8.5, 5, 1, -6))
    doorManager3.linkButton(level.addButtonAt(9.5, 8))
    level.addBlockAt(13.5, 7.5);
}, function(level){
    level.addPumpkinAt(3.5, 5.4);
}),
/*
@name "Sugar Rush"
@background "level-6-background"

control #fac
control #fd4
control #caf
control #4fc
control #aaa

platform 4 3.5 8 .5
box 5 4
box 8 2.5
platform 10 5 1 2
platform 12 0 1 4
platform 3 5 4 0.5
platform 7 5 1 2

door #fac 11 5 1 -2
block 12 4
platform 12 5 2 1
platform 13 2 1 2
button #fac 13 4
platform 4 3 1 1

button #fd4 0 6
door #fd4 4 4 1 1

door #aaa 6 4 1 1
button #aaa 5 2.5

door #4fc 13 1 1 -1
button #4fc 8 2.5
button #4fc 11 2.5

box 13 0

button #caf 13 0
door #caf 8 5 2 -2

pumpkin 6 6
*/
new Level("Sugar Rush", document.getElementById("level-6-background"), function(level, player, berry){
    level.addPlatformAt(9, 4.75, 8, 0.5);
    level.addPlatformAt(11.5, 7, 1, 2);
    level.addPlatformAt(13.5, 3, 1, 4);
    level.addPlatformAt(6, 6.25, 4, 0.5);
    level.addPlatformAt(8.5, 7, 1, 2);
    level.addPlatformAt(14, 6.5, 2, 1);
    level.addPlatformAt(14.5, 4, 1, 2);
    level.addPlatformAt(5.5, 4.5, 1, 1);
    level.addBoxAt(6.5, 5.5);
    level.addBoxAt(9.5, 4);
    level.addBoxAt(14.5, 1.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(12.5, 7, 1, -2))
    doorManager0.linkButton(level.addButtonAt(14.5, 6))
    let doorManager1 = level.addDoorManager("#fd4");
    doorManager1.linkDoor(level.addDoorAt(5.5, 5.5, 1, 1))
    doorManager1.linkButton(level.addButtonAt(1.5, 8))
    let doorManager2 = level.addDoorManager("#caf");
    doorManager2.linkDoor(level.addDoorAt(10, 7, 2, -2))
    doorManager2.linkButton(level.addButtonAt(14.5, 2))
    let doorManager3 = level.addDoorManager("#4fc");
    doorManager3.linkDoor(level.addDoorAt(14.5, 2.5, 1, -1))
    doorManager3.linkButton(level.addButtonAt(9.5, 4.5))
    doorManager3.linkButton(level.addButtonAt(12.5, 4.5))
    let doorManager4 = level.addDoorManager("#aaa");
    doorManager4.linkDoor(level.addDoorAt(7.5, 5.5, 1, 1))
    doorManager4.linkButton(level.addButtonAt(6.5, 4.5))
    level.addBlockAt(13.5, 5.5);
}, function(level){
    level.addPumpkinAt(7.5, 7.4);
}),
/*
@name "Mind Games"
@background "level-7-background"

control #fac
control #4fc
control #caf
control #fd4
control #ccc

button #ccc 0 6

door #caf 1 4 1 -3

button #fd4 2 3
button #caf 2 6

platform 2 4 2 .5
box 2 6
box 2 3
door #4fc 4 4 2 -3


platform 6 4 2 .5

block 7 3

door #fac 7 0 1 3
platform 7 4 1 2

block 7 6
block 10 3

button #4fc 9 6
button #fac 8 6

door #fd4 10 0 1 3

door #ccc 11 0 1 -7
platform 10 4 1 3

pumpkin 5.5 1
*/
new Level("Mind Games", document.getElementById("level-7-background"), function(level, player, berry){
    level.addPlatformAt(4, 5.25, 2, 0.5);
    level.addPlatformAt(8, 5.25, 2, 0.5);
    level.addPlatformAt(8.5, 6, 1, 2);
    level.addPlatformAt(11.5, 6.5, 1, 3);
    level.addBoxAt(3.5, 7.5);
    level.addBoxAt(3.5, 4.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(8.5, 2.5, 1, 3))
    doorManager0.linkButton(level.addButtonAt(9.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(6, 6.5, 2, -3))
    doorManager1.linkButton(level.addButtonAt(10.5, 8))
    let doorManager2 = level.addDoorManager("#caf");
    doorManager2.linkDoor(level.addDoorAt(2.5, 6.5, 1, -3))
    doorManager2.linkButton(level.addButtonAt(3.5, 8))
    let doorManager3 = level.addDoorManager("#fd4");
    doorManager3.linkDoor(level.addDoorAt(11.5, 2.5, 1, 3))
    doorManager3.linkButton(level.addButtonAt(3.5, 5))
    let doorManager4 = level.addDoorManager("#ccc");
    doorManager4.linkDoor(level.addDoorAt(12.5, 4.5, 1, -7))
    doorManager4.linkButton(level.addButtonAt(1.5, 8))
    level.addBlockAt(8.5, 4.5);
    level.addBlockAt(8.5, 7.5);
    level.addBlockAt(11.5, 4.5);
}, function(level){
    level.addPumpkinAt(7, 2.4);
}),
/*
@name "Two Pillars"
@background "level-8-background"

control #fac
control #4fc
control #fd4

button #fac 0 6
door #4fc 3 4 1 -3
platform 4 4 1 3
platform 2 5 1 2
box 2 4
button #4fc 2 4

button #fac 5 6

button #fd4 4 3

door #fac 6 0 1 4
box 7.5 6
door #4fc 6 4 1 -3
door #4fc 9 4 1 -3

button #fac 10 6


platform 11 2 1 5

door #fac 9 0 1 4
door #fd4 11 0 1 2

pumpkin 5 5.5
*/
new Level("Two Pillars", document.getElementById("level-8-background"), function(level, player, berry){
    level.addPlatformAt(5.5, 6.5, 1, 3);
    level.addPlatformAt(3.5, 7, 1, 2);
    level.addPlatformAt(12.5, 5.5, 1, 5);
    level.addBoxAt(3.5, 5.5);
    level.addBoxAt(9, 7.5);
    let doorManager0 = level.addDoorManager("#fac");
    doorManager0.linkDoor(level.addDoorAt(7.5, 3, 1, 4))
    doorManager0.linkDoor(level.addDoorAt(10.5, 3, 1, 4))
    doorManager0.linkButton(level.addButtonAt(1.5, 8))
    doorManager0.linkButton(level.addButtonAt(6.5, 8))
    doorManager0.linkButton(level.addButtonAt(11.5, 8))
    let doorManager1 = level.addDoorManager("#4fc");
    doorManager1.linkDoor(level.addDoorAt(4.5, 6.5, 1, -3))
    doorManager1.linkDoor(level.addDoorAt(7.5, 6.5, 1, -3))
    doorManager1.linkDoor(level.addDoorAt(10.5, 6.5, 1, -3))
    doorManager1.linkButton(level.addButtonAt(3.5, 6))
    let doorManager2 = level.addDoorManager("#fd4");
    doorManager2.linkDoor(level.addDoorAt(12.5, 2, 1, 2))
    doorManager2.linkButton(level.addButtonAt(5.5, 5))
}, function(level){
    level.addPumpkinAt(6.5, 6.9);
}),
/*
@name "Remote Control"
@background "level-9-background"

# Left block
control #4fc
control #fac
control #fd4

button #fd4 1 6
button #fac 2 6
button #4fc 3 6
door #4fc 4 5 1 -2

platform 0 5 4 0.5
platform 5 5 1 2.5

pumpkin 0 4
box 5 4

# Top block

platform 9 -1 1 4

box 10 0

door #fac 10 1 1 -1
door #4fc 10 2 1 -1

platform 11 -1 1 4

# Bottom block

platform 8 5 2 .5

button #fac 9 6

door #fac 10 5 1 -2
door #fd4 11 5 1 -2

block 11 3
block 11 5
block 11 6

platform 13 4 2 1
button #fd4 13 3

control #caf
button #caf 12 6
door #caf 11 4 2 -0.5
box 12 3
*/
new Level("Remote Control", document.getElementById("level-9-background"), function(level, player, berry){
    level.addPlatformAt(3, 6.25, 4, 0.5);
    level.addPlatformAt(6.5, 7.25, 1, 2.5);
    level.addPlatformAt(10.5, 2, 1, 4);
    level.addPlatformAt(12.5, 2, 1, 4);
    level.addPlatformAt(10, 6.25, 2, 0.5);
    level.addPlatformAt(15, 5.5, 2, 1);
    level.addBoxAt(6.5, 5.5);
    level.addBoxAt(11.5, 1.5);
    level.addBoxAt(13.5, 4.5);
    let doorManager0 = level.addDoorManager("#4fc");
    doorManager0.linkDoor(level.addDoorAt(5.5, 7, 1, -2))
    doorManager0.linkDoor(level.addDoorAt(11.5, 3.5, 1, -1))
    doorManager0.linkButton(level.addButtonAt(4.5, 8))
    let doorManager1 = level.addDoorManager("#fac");
    doorManager1.linkDoor(level.addDoorAt(11.5, 2.5, 1, -1))
    doorManager1.linkDoor(level.addDoorAt(11.5, 7, 1, -2))
    doorManager1.linkButton(level.addButtonAt(3.5, 8))
    doorManager1.linkButton(level.addButtonAt(10.5, 8))
    let doorManager2 = level.addDoorManager("#fd4");
    doorManager2.linkDoor(level.addDoorAt(12.5, 7, 1, -2))
    doorManager2.linkButton(level.addButtonAt(2.5, 8))
    doorManager2.linkButton(level.addButtonAt(14.5, 5))
    let doorManager3 = level.addDoorManager("#caf");
    doorManager3.linkDoor(level.addDoorAt(13, 5.25, 2, -0.5))
    doorManager3.linkButton(level.addButtonAt(13.5, 8))
    level.addBlockAt(12.5, 4.5);
    level.addBlockAt(12.5, 6.5);
    level.addBlockAt(12.5, 7.5);
}, function(level){
    level.addPumpkinAt(1.5, 5.4);
})
]